...
def return_colours(labels):
    ...
    """
    colours=[]
    label_dict = {"H":"red", "S":"blue", "F":"black"}
...
def plot_shift_results(datafile, params):
    ...
    legend_handles = [
        mpatches.Patch(color='red', label='Helices'),
        mpatches.Patch(color='blue', label='Sheets'),
        mpatches.Patch(color='black', label='Flexible'),
    ]
    
    fig.legend(handles=legend_handles, ncol=3, loc='upper center', bbox_to_anchor=(0.5, 0.95))
...
def plot_J3_scatter(datafile):
    ...
    figure.legend(handles=legend_handles, loc='upper center', bbox_to_anchor=(0.5, 0.95), ncol=3)
...
def stats(datafile, params, whichss, prop):
    ...
    return chi_sq, rmsd, r2, avg_rel_err, mean_nmr, mean_md
