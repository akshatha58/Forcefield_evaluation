...
def H(r):
	i = np.argmax(r.atoms.names == 'HN') # For CHARMM Trajectories
	# i = np.argmax(r.atoms.names == 'H') # For Amber Trajectories
	# print("H ",i)
	return r.atoms[i]
...
def process_frame(u, residues, prot, notH):
	...
	for r in residues:
		...
		# print("N(r), H(r) ", N(r).position, H(r).position)
	...
	for (i,r) in enumerate(res):
		...
		if i >= startch+2:
			if ('OXT' in r.atoms.names or 'OT2' in r.atoms.names) and len(residues) > 0: #end of chain
				...
				residues.append(r)
	# print("residues extracted:", residues)
	return residues[:-1]
